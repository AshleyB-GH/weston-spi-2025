...
const ANS_SPIS = RAW.ans_spis;
const ADR_SPIS = RAW.adr_spis;
const SPI_LABELS = RAW.spi_labels;
...
export default function App() {
  ...
  const quarters2025 = RAW.quarters.filter(q => q.startsWith("2025"));
  ...
  const barData = useMemo(() => activeQuarters.map(q => ({
    quarter: q,
    ANS: RAW.summary[q]?.ans || 0,
    ADR: RAW.summary[q]?.adr || 0,
    Misc: RAW.summary[q]?.misc || 0,
  })), [activeQuarters]);

  const ansLineData = useMemo(() => activeQuarters.map(q => {
    const s = RAW.summary[q] || {};
    const obj = { quarter: q };
  ...
  const spiBarData = useMemo(() => {
    ...
    return all.map(k => ({
      name: SPI_LABELS[k],
      code: k,
      count: activeQuarters.reduce((s, q) => s + (RAW.summary[q]?.[k] || 0), 0),
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,marginBottom:20}}>
          {[{unit:"ANS",spis:ANS_SPIS,color:ANS_COLOR,bg:"#e0f2fe"},{unit:"ADR",spis:ADR_SPIS,color:ADR_COLOR,bg:"#fff7ed"}].map(({unit,spis,color,bg}) => (
            <div key={unit} className="chart-card">
              ...
              {spis.map((k,i) => {
                const cnt = activeQuarters.reduce((s,q) => s+(RAW.summary[q]?.[k]||0),0);
